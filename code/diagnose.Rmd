...
# What the fuck is going on with the data
...
explore.data <- function(dat, params) {
  ...
  if (params$save == TRUE) {
    # Save in old server
    #path.dir <- '/run/user/1000/gvfs/smb-share:server=fs01.isglobal.lan,share=data/ws_helix/HELIX_analyses/HELIX_Panel_Nonpersistent_multi-omics/'
    #path.dir <- paste0(path.dir, "data/", params$file.name)
...
meta.1 <- purrr::discard(meta.all.1, is.null) %>% purrr::reduce(dplyr::inner_join, by = "SampleID")

meta.2 <- purrr::discard(meta.all.2, is.null) %>% purrr::reduce(dplyr::inner_join, by = "SampleID")
...
# Common subjects by time point separately
